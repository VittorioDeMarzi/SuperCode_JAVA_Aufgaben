...
DROP TABLE IF EXISTS Zugverbindung;

CREATE TABLE Zugverbindung (
    VerbindungID INT PRIMARY KEY,
    ZugID INT,
    Startort VARCHAR(50),
    Ankunftsort VARCHAR(50),
    Datum DATE,
    Dauer TIME,
...
SELECT STARTORT, COUNT(*) as ANZAHL
FROM ZUGVERBINDUNG
GROUP BY STARTORT
...
SELECT ZUGID, COUNT(DISTINCT DATUM)
FROM ZUGVERBINDUNG v1
GROUP BY ZUGID
HAVING COUNT(*) >= 2;
